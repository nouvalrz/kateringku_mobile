...
class CateringHomeController extends GetxController implements GetxService{
  ...
  CateringHomeController({required this.cateringProductRepo, required this.cartRepo});
  ...
  void addProductQuantity(int index, {bool fromCart = false}){
    if(products[index].orderQuantity >= products[index].maximumQuantity!){
      showCustomSnackBar(message: "Jumlah maksimal adalah ${products[index].maximumQuantity!}", title: "JUMLAH TELAH MAKSIMAL");
      return;
    }
    if(fromCart){
    ...
    else if(products[index].minimumQuantity! >= 1 && products[index].orderQuantity == 0){
      totalPrice.value += products[index].fixPrice() * products[index].minimumQuantity!;
  ...
  Future<void> postCart() async{
    ...
    if (allOrderProducts.isNotEmpty) {
      cartRequest['products'] = allOrderProducts.map((v) => v.toJson()).toList();
    }

    Response response = await cartRepo.postCart(cartRequest);

    if(response.statusCode == 200){
      showCustomSnackBar(message: "Anda berhasil menambahkan order ke keranjang!", title: "Tambah Keranjang Berhasil");
